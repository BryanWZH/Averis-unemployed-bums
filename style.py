...
def theme_switch_html():
    ...
    return ("<style>html,body{margin:0;background:transparent;overflow:hidden}"
            ...
            "var p=window.parent,b=document.getElementById('b'),dark=false;"
            "function sync(){try{var cs=p.getComputedStyle(p.document.querySelector('.stApp'));"
            "dark=cs.colorScheme==='dark';b.style.color=dark?'#E7EDF6':'#2E2417';"
            "b.style.borderColor=dark?'rgba(231,237,246,.45)':'rgba(46,36,23,.40)';"
            "var t=dark?String.fromCodePoint(0x2600,0xFE0F)+'  Switch to light':String.fromCodePoint(0x1F319)+'  Switch to dark';"
            "if(b.textContent!==t)b.textContent=t;}catch(e){}}"
